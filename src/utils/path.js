...
export const removeTrailingSlash = (val) => (val?.endsWith("/") ? val?.substring(0, val.length - 1) : val);
...
export function generateAsPath({ query, pathname, pathParams }) {
    ...
    if (!stringifyQuery) return pathname;
...
export const doesPathInPaths = (currentPath, paths) => paths.some((path) => matchPath(path, currentPath));
